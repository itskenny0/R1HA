...
- **Energy**: DRAW (sum of every `device_class=power` sensor), PRODUCTION (heuristic sum of solar / pv / grid_export / production sensors), TODAY's kWh (sum of `device_class=energy` `total_increasing` sensors), and the top 5 current consumers ranked by W draw. WATER and GAS daily-total tiles surface when `device_class=water` / `device_class=gas` sensors exist. An EXPORT CSV action shares the current draw, today's totals, the consumer ranking, and the consumption history as a CSV. Native: no WebView, no Lovelace bundle.
